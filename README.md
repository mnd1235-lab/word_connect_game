...
비밀번호를 바꿀 때는 스크립트를 쓴다. 다른 값은 건드리지 않고, 중복된
`APP_PASSWORD` 줄을 정리하고, BOM 없이 저장한 뒤 파싱까지 확인한다.
...
플레이하면서 `PILOT-LOG.md`를 손으로 채운다. 숫자를 자동으로 모으지 않는 것이
방침이다(PRD 8장). 판정 한 건마다 터미널과 클라우드 앱 로그에 `[submit]`,
`[judge]` 로그가 한 줄씩 찍히므로 경로와 소요 시간은 거기서 옮겨 적는다.
